...
public struct Driver {
  ...
  lazy var prefixMapping: [(AbsolutePath, AbsolutePath)] = {
    ...
    if !mapping.isEmpty {
      ...
      if objcGeneratedHeaderPath != nil && useExternalBridgingHeader {
        // Only warn if the bridging header path is actually covered by one of
        // the prefix mappings; otherwise it cannot leak a mapped path into the
        // generated header. When the path cannot be resolved to an absolute
        // path, conservatively assume it could be affected.
        let bridgingHeaderPath = originalObjCHeaderFile.flatMap { handle -> AbsolutePath? in
          guard let cwd = workingDirectory ?? fileSystem.currentWorkingDirectory else {
            return nil
          }
          return VirtualPath.lookup(handle).resolvedRelativePath(base: cwd).absolutePath
        }
        let isBridgingHeaderPrefixMapped = bridgingHeaderPath.map { path in
          mapping.contains { path.isDescendantOfOrEqual(to: $0.0) }
        } ?? true
        if isBridgingHeaderPrefixMapped {
          diagnosticEngine.emit(.warning("generated Objective-C header can contain a prefix mapped bridging header path"))
          diagnosticEngine.emit(.note("the bridging header path in the header generated by '-emit-objc-header' is mapped by '-scanner-prefix-*' options and might not be resolvable by the consumers of the generated header"))
          if isFrontendArgSupported(.internalImportBridgingHeader) {
            diagnosticEngine.emit(.note("adopt '-internal-import-bridging-header' option instead of '-import-bridging-header' or '-import-objc-header' to keep the bridging header out of the generated header"))
          }
        }
      }
    }
