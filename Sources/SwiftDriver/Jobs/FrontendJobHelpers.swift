...
extension Driver {
  ...
  mutating func addFrontendSupplementaryOutputArguments(commandLine: inout [Job.ArgTemplate],
    ...
    func processInputsForSupplementaryOutput(inputs: [TypedVirtualPath], outputType: FileType, flag: String, directory: String?, isSingleThreadedWMO: Bool = false) throws {
      ...
      for inputFile in inputs {
        ...
        if let outputFileMapPath = try outputFileMap?.existingOutput(inputFile: inputFile.fileHandle, outputType: outputType) {
          ...
          // For opt-records with explicit flag but no file map entry, derive path from input filename
          let outputPath = try generateSupplementaryOutputPath(for: inputFile, outputType: outputType, directory: ".")
          flaggedInputOutputPairs.append((flag: flag, input: inputFile, output: outputPath))
        }
      }
    ...
    func addOutputOfType(
      ...
      if finalOutputPath == nil && (outputType == .sil || outputType == .llvmIR || outputType.isOptimizationRecord) {
        ...
        if directory != nil || hasFileMapEntries || (parsedOptions.hasArgument(.saveTemps) && !hasFileMapEntries && !outputType.isOptimizationRecord) || hasOptRecordFlag {
          ...
          let isSingleThreadedWMO = !compilerMode.usesPrimaryFileInputs && numThreads < 2
          try processInputsForSupplementaryOutput(inputs: inputsToProcess, outputType: outputType, flag: flag, directory: directory, isSingleThreadedWMO: isSingleThreadedWMO)
          return
        }
      }
