...
@Suite struct MiscDriverTests {
  ...
  @Test func invalidModuleAliasing() async throws {
    try await assertDriverDiagnostics(
      args: [
        "swiftc", "foo.swift", "-module-name", "Foo", "-module-alias", "CarBar", "-emit-module", "-emit-module-path",
        "/tmp/dir/Foo.swiftmodule",
      ]
    ) {
    ...
    try await assertDriverDiagnostics(
      args: [
        "swiftc", "foo.swift", "-module-name", "Foo", "-module-alias", "Foo=Bar", "-emit-module", "-emit-module-path",
        "/tmp/dir/Foo.swiftmodule",
      ]
    ) {
    ...
    try await assertDriverDiagnostics(
      args: [
        "swiftc", "foo.swift", "-module-name", "Foo", "-module-alias", "Bar=C-ar", "-emit-module", "-emit-module-path",
        "/tmp/dir/Foo.swiftmodule",
      ]
    ) {
    ...
    try await assertDriverDiagnostics(
      args: [
        "swiftc", "foo.swift", "-module-name", "Foo", "-module-alias", "C`ar=Bar", "-emit-module", "-emit-module-path",
        "/tmp/dir/Foo.swiftmodule",
      ]
    ) {
      $1.expect(.error("module name \"C`ar\" is not a valid identifier"))
    ...
    try await assertDriverDiagnostics(
      args: [
        "swiftc", "foo.swift", "-module-name", "Foo", "-module-alias", "Car=Bar", "-module-alias", "Train=Car",
        "-emit-module", "-emit-module-path", "/tmp/dir/Foo.swiftmodule",
      ]
    ) {
    ...
    try await assertDriverDiagnostics(
      args: [
        "swiftc", "foo.swift", "-module-name", "Foo", "-module-alias", "Car=Bar", "-module-alias", "Car=Bus",
        "-emit-module", "-emit-module-path", "/tmp/dir/Foo.swiftmodule",
      ]
    ) {
      $1.expect(.error("the name \"Car\" is already used for a module alias or an underlying name"))
  ...
  @Test func packageInterfacePathImplicit() async throws {
    ...
    do {
      var driver = try TestDriver(
        args: [
          "swiftc", "foo.swift", "-emit-module", "-module-name", "foo",
    ...
    do {
      var driver = try TestDriver(
        args: [
          "swiftc", "foo.swift", "-emit-module", "-module-name", "foo", "-library-level", "api",
          "-emit-module-interface", "-enable-library-evolution",
        ],
        env: envVars
      )
  ...
  @Test func profileArgValidation() async throws {
    try await assertDriverDiagnostics(args: [
      "swiftc", "foo.swift", "-profile-generate", "-profile-use=profile.profdata",
    ]) {
    ...
    try await withTemporaryDirectory { path in
      try localFileSystem.writeFileContents(path.appending(component: "profile.profdata"), bytes: .init())
      try await assertDriverDiagnostics(args: [
        "swiftc", "-working-directory", path.pathString, "foo.swift",
      ...
      try await assertDriverDiagnostics(args: [
        ...
        $1.expect(
          .error(
            Driver.Error.missingProfilingData(
              path.appending(component: "profile.profdata,profile2.profdata").pathString
            )
          )
        )
      }
  ...
  @Test func profileLinkerArgs() async throws {
    ...
    do {
      var driver = try TestDriver(
        args: ["swiftc", "-profile-generate", "-target", "x86_64-apple-macosx10.9", "test.swift"],
        env: envVars
      )
      let plannedJobs = try await driver.planBuild()
    ...
    do {
      var driver = try TestDriver(
        args: ["swiftc", "-profile-generate", "-target", "x86_64-apple-ios7.1-simulator", "test.swift"],
        env: envVars
      )
      let plannedJobs = try await driver.planBuild()
    ...
    do {
      var driver = try TestDriver(
        args: ["swiftc", "-profile-generate", "-target", "arm64-apple-ios7.1", "test.swift"],
        env: envVars
      )
      let plannedJobs = try await driver.planBuild()
    ...
    do {
      var driver = try TestDriver(
        args: ["swiftc", "-profile-generate", "-target", "x86_64-apple-tvos9.0-simulator", "test.swift"],
        env: envVars
      )
      let plannedJobs = try await driver.planBuild()
    ...
    do {
      var driver = try TestDriver(
        args: ["swiftc", "-profile-generate", "-target", "arm64-apple-tvos9.0", "test.swift"],
        env: envVars
      )
      let plannedJobs = try await driver.planBuild()
    ...
    do {
      var driver = try TestDriver(
        args: ["swiftc", "-profile-generate", "-target", "i386-apple-watchos2.0-simulator", "test.swift"],
        env: envVars
      )
      let plannedJobs = try await driver.planBuild()
    ...
    do {
      var driver = try TestDriver(
        args: ["swiftc", "-profile-generate", "-target", "armv7k-apple-watchos2.0", "test.swift"],
        env: envVars
      )
      let plannedJobs = try await driver.planBuild()
    ...
    do {
      var driver = try TestDriver(args: [
        "swiftc", "-profile-generate", "-use-ld=link", "-target", "x86_64-unknown-windows-msvc", "test.swift",
      ])
      let plannedJobs = try await driver.planBuild()
  ...
  @Test func conditionalCompilationArgValidation() async throws {
    try await assertDriverDiagnostics(args: ["swiftc", "foo.swift", "-DFOO=BAR"]) {
      $1.expect(
        .warning(
          "conditional compilation flags do not have values in Swift; they are either present or absent (rather than 'FOO=BAR')"
        )
      )
    }
  ...
  @Test func frameworkSearchPathArgValidation() async throws {
    try await assertDriverDiagnostics(args: ["swiftc", "foo.swift", "-F/some/dir/xyz.framework"]) {
      $1.expect(
        .warning(
          "framework search path ends in \".framework\"; add directory containing framework instead: /some/dir/xyz.framework"
        )
      )
    }
  ...
  @Test func multipleValidationFailures() async throws {
    try await assertDiagnostics { engine, verifier in
      ...
      _ = try TestDriver(
        args: ["swiftc", "foo.swift", "-Dnot-an-identifier", "-F/some/dir/xyz.framework"],
  ...
  @Test func dotFileEmission() async throws {
    var driver = try TestDriver(args: [
      ...
      "-experimental-emit-module-separately", "-working-directory",
      localFileSystem.currentWorkingDirectory!.description,
    ])
    let plannedJobs = try await driver.planBuild()
  ...
  @Test func regressions() async throws {
    ...
    if !driver.targetTriple.isDarwin {
      #expect(
        !plannedJobs[1].commandLine.contains(subsequence: [
          "-L", .path(.absolute(try .init(validating: "/usr/lib/swift"))),
        ])
      )
    }
  }
  ...
  @Test func diagnosticOptions() async throws {
    ...
    do {
      var driver = try TestDriver(args: [
        "swift", "-warnings-as-errors", "-no-warnings-as-errors", "-suppress-warnings", "-suppress-remarks",
        "foo.swift",
      ])
      let plannedJobs = try await driver.planBuild()
      #expect(plannedJobs.count == 1)
    ...
    do {
      try await assertDriverDiagnostics(args: [
        "swift", "-no-warnings-as-errors", "-warnings-as-errors", "-suppress-warnings", "foo.swift",
      ]) {
  ...
  @Test func deterministicCheck() async throws {
    do {
      var driver = try TestDriver(args: [
        "swiftc", "-enable-deterministic-check", "foo.swift",
  ...
  @Test func prebuiltModuleCacheFlags() async throws {
    ...
    do {
      let resourceDirPath: String = try testInputsPath.appending(
        components: "PrebuiltModules-macOS10.15.xctoolchain",
        "usr",
        "lib",
        "swift"
      ).nativePathString(escaped: false)

      var driver = try TestDriver(
        args: [
          "swiftc", "-target", "x86_64-apple-ios13.1-macabi", "foo.swift", "-sdk", mockSDKPath, "-resource-dir",
          resourceDirPath,
        ],
        env: envVars
      )
    ...
    do {
      let resourceDirPath: String = try testInputsPath.appending(
        components: "PrebuiltModules-macOSUnversioned.xctoolchain",
        "usr",
        "lib",
        "swift"
      ).nativePathString(escaped: false)
  ...
  @Test func relativeInputs() async throws {
    do {
      ...
      try expectJobInvocationMatches(
        compileJob,
        .flag("-resource-dir"),
        toPathOption("relresourcepath", isRelative: true)
      )
    ...
    do {
      ...
      expectJobInvocationMatches(
        compileJob,
        .flag("-primary-file"),
        .path(.absolute(workingDirectory.appending(component: "foo.swift")))
      )
      expectJobInvocationMatches(
        compileJob,
        .flag("-resource-dir"),
        .path(.absolute(workingDirectory.appending(component: "relresourcepath")))
      )
      expectJobInvocationMatches(
        compileJob,
        .flag("-sdk"),
        .path(.absolute(workingDirectory.appending(component: "relsdkpath")))
      )
    }
    ...
    try await withTemporaryDirectory { dir in
      ...
      try expectJobInvocationMatches(
        compileJob,
        .flag("-o"),
        .path(.absolute(.init(validating: "/tmp/foo/.build/x86_64-apple-macosx/debug/foo.build/foo.o")))
      )
    }

    try await withTemporaryDirectory { dir in
      let fileMapFile = dir.appending(component: "file-map-file")
      let outputMapContents: ByteString = .init(
        encodingAsUTF8: """
          {
            "": {
              "diagnostics": "/tmp/foo/.build/x86_64-apple-macosx/debug/foo.build/main.dia",
  ...
  @Test func sysrootHandling() async throws {
    do {
      ...
      try expectJobInvocationMatches(
        jobs[0],
        .flag("-sysroot"),
        .path(.absolute(.init(validating: "/path/to/sysroot")))
      )
    }

    do {
      var driver = try TestDriver(args: [
        "swiftc", "-sdk", "/path/to/sdk", "-sysroot", "/path/to/sysroot", "-c", "input.swift",
      ])
      let jobs = try await driver.planBuild()
  ...
  @Test func clangModuleValidateOnce() async throws {
    ...
    guard flagTest.isFrontendArgSupported(.clangBuildSessionFile),
      flagTest.isFrontendArgSupported(.validateClangModulesOnce)
    else {
      return
    }
    ...
    do {
      try await assertDriverDiagnostics(args: [
        ...
        $1.expect(
          .error("'-validate-clang-modules-once' cannot be specified if '-clang-build-session-file' is not present")
        )
      }
    }
  ...
  @Test func webAssemblyUnsupportedFeatures() async throws {
    ...
    do {
      var driver = try TestDriver(
        args: ["swiftc", "-target", "wasm32-unknown-wasi", "-emit-library", "foo.swift"],
        env: env
      )
      await #expect {
        try await driver.planBuild()
    ...
    do {
      var driver = try TestDriver(
        args: ["swiftc", "-target", "wasm32-unknown-wasi", "-no-static-executable", "foo.swift"],
        env: env
      )
      await #expect {
        try await driver.planBuild()
  ...
  @Test func experimentalPerformanceAnnotations() async throws {
    do {
      var driver = try TestDriver(args: [
        "swiftc", "foo.swift", "-experimental-performance-annotations",
        "-emit-sil", "-o", "foo.sil",
      ])
      let plannedJobs = try await driver.planBuild()
      #expect(plannedJobs.count == 1)
  ...
  @Test func loadPackageInterface() async throws {
    try await withTemporaryDirectory { path in
      ...
      var driver = try TestDriver(
        args: [
          "swiftc", main.nativePathString(escaped: false),
          "-typecheck",
          "-package-name", "foopkg",
  ...
  @Test func vfsOverlay() async throws {
    ...
    do {
      var driver = try TestDriver(args: [
        "swiftc", "-c", "-vfsoverlay", "overlay1.yaml", "-vfsoverlay", "overlay2.yaml", "-vfsoverlay", "overlay3.yaml",
        "foo.swift",
      ])
      let plannedJobs = try await driver.planBuild().removingAutolinkExtractJobs()
      #expect(plannedJobs.count == 1)
      ...
      try expectJobInvocationMatches(
        plannedJobs[0],
        .flag("-vfsoverlay"),
        toPathOption("overlay1.yaml"),
        .flag("-vfsoverlay"),
        toPathOption("overlay2.yaml"),
        .flag("-vfsoverlay"),
        toPathOption("overlay3.yaml")
      )
    }
  }
          ...
          emitModuleJob.commandLine.contains(subsequence: [
            ...
            .path(
              .absolute(
                path.appending(
                  components: "SDKDB",
                  "Test.\(driver.frontendTargetInfo.target.moduleTriple.triple).swift.sdkdb"
                )
              )
            ),
          ])
  ...
  @Test(.requireHostOS(.macosx, comment: "ABI descriptor is only emitted on Darwin platforms")) func emitABIDescriptor()
    async throws
  {
    do {
      var driver = try TestDriver(args: [
        "swiftc", "-module-name=ThisModule", "-wmo", "main.swift", "multi-threaded.swift", "-emit-module", "-o",
        "test.swiftmodule",
      ])
      let plannedJobs = try await driver.planBuild().removingAutolinkExtractJobs()
    ...
    do {
      var driver = try TestDriver(args: [
        "swiftc", "-module-name=ThisModule", "main.swift", "multi-threaded.swift", "-emit-module", "-o",
        "test.swiftmodule", "-experimental-emit-module-separately",
      ])
      let plannedJobs = try await driver.planBuild().removingAutolinkExtractJobs()
