...
@Suite struct SanitizerTests {
  ...
  @Test func sanitizerRecoverArgs() async throws {
    ...
    do {
      ...
      try await assertDriverDiagnostics(args: commonArgs + ["-sanitize=undefined", "-sanitize=address", "-sanitize-recover=address"]) {
        $1.forbidUnexpected(.error, .warning)
      }
    }
    do {
      // comma sanitizer + address sanitizer recover together
      try await assertDriverDiagnostics(args: commonArgs + ["-sanitize=undefined,address", "-sanitize-recover=address"]) {
        $1.forbidUnexpected(.error, .warning)
      }
    }
    #endif
  ...
  private static func hasSanitizerRuntime() -> Bool {
      guard var driver = try? TestDriver(args: ["swiftc", "foo.swift", "bar.swift", "-emit-executable", "-module-name", "Test", "-use-ld=lld", "-sanitize=address"]) else {
        return false
      }
      guard let exist = try? driver.unwrap({
        try $0.toolchain.runtimeLibraryExists(for: .address,
  ...
  @Test func sanitizerCoverageArgs() async throws {
    ...
    try await assertNoDriverDiagnostics(args: "swiftc", "foo.swift", "-sanitize=thread", "-sanitize-coverage=edge,indirect-calls,trace-bb,trace-cmp,8bit-counters,pc-table,inline-8bit-counters")
#endif
  }

  @Test func sanitizerAddressUseOdrIndicator() async throws {
    do {
      var driver = try TestDriver(args: ["swiftc", "-sanitize=address", "-sanitize-address-use-odr-indicator", "Test.swift"])
    ...
    do {
      try await assertDriverDiagnostics(args: ["swiftc", "-sanitize=thread", "-sanitize-address-use-odr-indicator", "Test.swift"]) {
  ...
  @Test(.requireHostOS(.darwin, comment: "-sanitize-stable-abi is only implemented on Darwin")) func sanitizeStableAbi() async throws {
