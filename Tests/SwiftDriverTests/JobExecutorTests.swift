    ...
    try withTemporaryDirectory { path in
      ...
      let link = Job(
        ...
        commandLine: [
          ...
          .path(try toolchain.clangRT.get().virtualPath),
          "-syslibroot", .path(.absolute(try toolchain.sdk.get())),
          "-lobjc", "-lSystem", "-arch", "x86_64",
          "-force_load", .path(try toolchain.compatibility50.get().virtualPath),
          "-force_load", .path(try toolchain.compatibilityDynamicReplacements.get().virtualPath),
          "-L", .path(try toolchain.resourcesDirectory.get().virtualPath),
