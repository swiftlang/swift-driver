...
@Suite struct APIDigesterTests {
  @Test func baselineGenerationRequiresTopLevelModule() async throws {
    ...
    try await assertDriverDiagnostics(args: ["swiftc", "foo.swift", "-emit-digester-baseline-path", "/output/path.abi.json"]) {
  ...
  @Test func digesterModeValidation() async throws {
    try await assertDriverDiagnostics(args: ["swiftc", "foo.swift", "-emit-module", "-emit-digester-baseline", "-digester-mode", "notamode"]) {
      $1.expect(.error("invalid value 'notamode' in '-digester-mode'"))
    }
    try await assertNoDriverDiagnostics(args: "swiftc", "foo.swift", "-emit-module", "-emit-digester-baseline", "-digester-mode", "api")
    try await assertNoDriverDiagnostics(args: "swiftc", "foo.swift", "-emit-module", "-emit-module-interface",
                                  "-enable-library-evolution", "-emit-digester-baseline", "-digester-mode", "abi")
  }

  @Test func abiDigesterRequirements() async throws {
    try await assertDriverDiagnostics(args: ["swiftc", "foo.swift", "-emit-module", "-emit-module-interface",
    ...
    try await assertDriverDiagnostics(args: ["swiftc", "foo.swift", "-emit-module",
                                       "-enable-library-evolution", "-emit-digester-baseline", "-digester-mode", "abi"]) {
  ...
  @Test func baselineOutputPath() async throws {
    ...
    do {
      var driver = try TestDriver(args: ["swiftc", "foo.swift", "-emit-module","-emit-module-interface", "-enable-library-evolution", "-emit-digester-baseline", "-digester-mode", "abi"])
    ...
    do {
      var driver = try TestDriver(args: ["swiftc", "foo.swift", "-emit-module", "-emit-digester-baseline-path", "bar.api.json"])
      let digesterJob = try #require(await driver.planBuild().first { $0.kind == .generateAPIBaseline })
      #expect(digesterJob.commandLine.contains(subsequence: ["-o", try toPathOption("bar.api.json")]))
    }
    do {
      var driver = try TestDriver(args: ["swiftc", "foo.swift", "-emit-module","-emit-module-interface", "-enable-library-evolution", "-digester-mode", "abi", "-emit-digester-baseline-path", "bar.abi.json"])
      let digesterJob = try #require(await driver.planBuild().first { $0.kind == .generateABIBaseline })
    ...
    do {
      try await withTemporaryDirectory { path in
        ...
        let digesterJob = try #require(await driver.planBuild().first { $0.kind == .generateAPIBaseline })
        #expect(digesterJob.commandLine.contains(subsequence: ["-o", .path(.absolute(projectDirPath.appending(component: "foo.api.json")))]))
      }
    }
    do {
      try await withTemporaryDirectory { path in
        ...
        var driver = try TestDriver(args: ["swiftc", "-emit-module",
                                       ...
                                       "-o", path.appending(component: "foo.swiftmodule").pathString])
        let digesterJob = try #require(await driver.planBuild().first { $0.kind == .generateABIBaseline })
        #expect(digesterJob.commandLine.contains(subsequence: ["-o", .path(.absolute(projectDirPath.appending(component: "foo.abi.json")))]))
      }
    }
    do {
      try await withTemporaryDirectory { path in
        ...
        var driver = try TestDriver(args: ["swiftc", "-wmo", "-emit-module",
                                       "-emit-module-interface", "-enable-library-evolution",
                                       path.appending(component: "foo.swift").pathString,
                                       "-emit-digester-baseline",
                                       "-digester-mode", "abi",
        ...
        let digesterJob = try #require(await driver.planBuild().first { $0.kind == .generateABIBaseline })
        #expect(digesterJob.commandLine.contains(subsequence: ["-o", .path(.absolute(try .init(validating: "/path/to/baseline.abi.json")))]))
      }
    }
    do {
      try await withTemporaryDirectory { path in
        ...
        let digesterJob = try #require(await driver.planBuild().first { $0.kind == .generateABIBaseline })
        #expect(digesterJob.commandLine.contains(subsequence: ["-o", .path(.absolute(try .init(validating: "/path/to/sourceinfo.abi.json")))]))
      }
    }
  }
  ...
  @Test func baselineGenerationJobFlags() async throws {
    do {
      ...
      #expect(digesterJob.commandLine.contains(subsequence: ["-sdk", .path(.absolute(try .init(validating: "/path/to/sdk")))]))
      #expect(digesterJob.commandLine.contains(subsequence: ["-I", .path(.absolute(try .init(validating: "/some/path")))]))
    ...
    do {
      var driver = try TestDriver(args: ["swiftc", "foo.swift", "-emit-module", "-emit-module-interface",
  ...
  @Test func comparisonOptionValidation() async throws {
    try await assertDriverDiagnostics(args: ["swiftc", "foo.swift",
  ...
  @Test(.requireHostOS(.macosx, comment: "ABI descriptor is only emitted on Darwin platforms")) func baselineComparisonJobFlags() async throws {
    do {
      var driver = try TestDriver(args: ["swiftc", "foo.swift", "-emit-module", "-compare-to-baseline-path", "/baseline/path",
                                     "-sdk", "/path/to/sdk", "-I", "/some/path", "-F", "framework/path",
      ...
      #expect(digesterJob.commandLine.contains(subsequence: ["-module", "foo"]))
      #expect(digesterJob.commandLine.contains(subsequence: ["-baseline-path", .path(.absolute(try .init(validating: "/baseline/path")))]))
      #expect(digesterJob.commandLine.contains(subsequence: ["-I", try toPathOption(".")]))
      #expect(digesterJob.commandLine.contains(subsequence: ["-sdk", .path(.absolute(try .init(validating: "/path/to/sdk")))]))
      #expect(digesterJob.commandLine.contains(subsequence: ["-I", .path(.absolute(try .init(validating: "/some/path")))]))
      #expect(digesterJob.commandLine.contains(subsequence: ["-F", try toPathOption("framework/path")]))
      #expect(digesterJob.commandLine.contains(subsequence: ["-breakage-allowlist-path",
    ...
    do {
      ...
      #expect(digesterJob.commandLine.contains("-diagnose-sdk"))
      #expect(digesterJob.commandLine.contains(subsequence: ["-input-paths", .path(.absolute(try .init(validating: "/baseline/path")))]))
      #expect(digesterJob.commandLine.contains(subsequence: ["-breakage-allowlist-path",
                                                                   try toPathOption("allowlist/path")]))
      #expect(digesterJob.commandLine.contains("-abi"))
...
@Suite struct APIDigesterBuildTests {
  ...
  @Test(.disabled("rdar://82302797"), .requireFrontendArgSupport(.disableFailOnError)) func apiComparisonEndToEnd() async throws {
    try await withTemporaryDirectory { path in
      let source = path.appending(component: "foo.swift")
      try localFileSystem.writeFileContents(source) {
        $0.send("""
      ...
      #expect(diags.diagnostics.map(\.text) == [
  ...
  @Test(.disabled("rdar://82302797"), .requireFrontendArgSupport(.disableFailOnError)) func abiComparisonEndToEnd() async throws {
    try await withTemporaryDirectory { path in
      let source = path.appending(component: "foo.swift")
