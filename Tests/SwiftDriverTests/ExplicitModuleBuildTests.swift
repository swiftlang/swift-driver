...
private func checkExplicitModuleBuildJob(
  job: Job,
  ...
  throws
{
  ...
  try checkExplicitModuleBuildJobDependencies(
    job: job,
...
private func checkExplicitModuleBuildJobDependencies(
  job: Job,
  moduleInfo: ModuleInfo,
  dependencyGraph: InterModuleDependencyGraph
) throws {
  let validateSwiftCommandLineDependency: (ModuleDependencyId, ModuleInfo) -> Void = { dependencyId, dependencyInfo in
    ...
    expectJobInvocationMatches(
      job,
      .flag("-swift-module-file=\(dependencyId.moduleName)=\(inputModulePath.description)")
    )
  }
  ...
  let validateClangCommandLineDependency:
    (
      ...
      ModuleInfo,
      ClangModuleDetails
    ) -> Void = { dependencyId, dependencyInfo, clangDependencyDetails in
      ...
      expectJobInvocationMatches(
        job,
        .flag("-fmodule-file=\(dependencyId.moduleName)=\(clangDependencyModulePathString)")
      )
    }
  ...
  for dependencyId in moduleInfo.allDependencies {
    ...
    case .swift(_), .swiftPrebuiltExternal(_):
...
internal func getDriverArtifactsForScanning() throws -> (
  stdLibPath: AbsolutePath,
  ...
  #expect(
    localFileSystem.exists(stdLibPath),
    "expected Swift StdLib at: \(stdLibPath.description)"
  )
  #expect(
    localFileSystem.exists(shimsPath),
...
func getStdlibShimsPaths(_ driver: Driver) throws -> (AbsolutePath, AbsolutePath) {
  ...
  if driver.targetTriple.isDarwin {
    ...
    if let sdkroot = try driver.toolchain.defaultSDKPath(driver.targetTriple) {
      return (
        sdkroot.appending(components: "usr", "lib", "swift", "windows"),
...
@Suite struct ExplicitModuleBuildTests {
    ...
    do {
      ...
      for job in modulePrebuildJobs {
        ...
        case .relative(try .init(validating: "SwiftShims.pcm")):
          try checkExplicitModuleBuildJob(
            job: job,
    ...
    try await withTemporaryDirectory { path in
      ...
      var driver = try TestDriver(
        args: [
          ...
          source1.nativePathString(escaped: false),
  ...
  @Test func bridgingHeaderDeps() async throws {
    ...
    try await withTemporaryDirectory { path in
      ...
      #expect(
        jsonDepsDecoded.contains { artifactInfo in
    ...
    try await withTemporaryDirectory { path in
      ...
      #expect(
        driver.diagnosticEngine.diagnostics.contains {
          $0.behavior == .warning
            && $0.message.text
              == "-auto-bridging-header-chaining requires generatePCH job, no chaining will be performed"
        }
      )
      #expect(!jobs.contains { $0.kind == .generatePCH })
  ...
  @Test(.requireScannerSupportsImportInfos()) func explicitImportDetails() async throws {
    try await withTemporaryDirectory { path in
      ...
      #expect(
        mainModuleImports.contains(
          ImportInfo(
            importIdentifier: "SwiftOnoneSupport",
            accessLevel: ImportInfo.ImportAccessLevel.Public,
            sourceLocations: []
          )
        )
          ...
          ImportInfo(
            ...
            sourceLocations: [
              ScannerDiagnosticSourceLocation(
                bufferIdentifier: main.nativePathString(escaped: false),
                lineNumber: 1,
                columnNumber: 15
              ),
              ScannerDiagnosticSourceLocation(
  ...
  @Test(.requireScannerSupportsLinkLibraries()) func explicitLinkLibraries() async throws {
    try await withTemporaryDirectory { path in
      ...
      for (depId, depInfo) in dependencyGraph.modules {
        switch depId {
        case .swiftPrebuiltExternal("Swift"), .swift("Swift"):
          try checkForLinkLibrary(depInfo, "swiftCore", false, false)
          break
        case .swiftPrebuiltExternal("_StringProcessing"), .swift("_StringProcessing"):
          try checkForLinkLibrary(depInfo, "swift_StringProcessing", false, false)
          break
        case .swiftPrebuiltExternal("SwiftOnoneSupport"), .swift("SwiftOnoneSupport"):
          try checkForLinkLibrary(depInfo, "swiftSwiftOnoneSupport", false, false)
          break
        case .swiftPrebuiltExternal("_Concurrency"), .swift("_Concurrency"):
  ...
  @Test func explicitModuleBuildJobs() async throws {
    ...
    try await withTemporaryDirectory { path in
      ...
      if try driver.interModuleDependencyOracle.supportsSeparateImportOnlyDependencies() {
        ...
        #expect(
          directImportedDependencies.contains(.swift("Swift"))
            || directImportedDependencies.contains(.swiftPrebuiltExternal("Swift"))
        )
        #expect(
          directImportedDependencies.contains(.swift("SwiftOnoneSupport"))
        ...
        if job.kind == .compile && driver.isFeatureSupported(.debug_info_explicit_dependency) {
          #expect(
            job.commandLine.contains(subsequence: [
              "-debug-module-path", try toPathOption("testExplicitModuleBuildJobs.swiftmodule"),
            ])
          )
        }
  ...
  @Test func registerModuleDependencyFlag() async throws {
    ...
    try await withTemporaryDirectory { path in
      ...
      #expect(
        dependencyGraph.modules.keys.contains(.swift("E")),
        "Module E should be in dependency graph when registered via -register-module-dependency"
      )
      #expect(
        dependencyGraph.modules.keys.contains(.swift("G")),
  ...
  @Test func explicitModuleBuildIncrementalEndToEnd() async throws {
    try await withTemporaryDirectory { path in
      ...
      var incrementalAlwaysRebuildDriver = try TestDriver(
        args: invocationArguments + ["-always-rebuild-module-dependencies"]
      )
      ...
      for job in jobs {
          ...
          outputExtension == FileType.pcm.rawValue
        {
          ...
          case .temporary(_):
            ...
            if outputFilePath.extension == FileType.object.rawValue
  ...
  @Test(.requireFrontendArgSupport(.moduleAlias)) func moduleAliasingPrebuiltWithScanDeps() async throws {
    try await withTemporaryDirectory { path in
      ...
      #expect(
        dependencyGraphA.modules.contains { (key: ModuleDependencyId, value: ModuleInfo) in
          key.moduleName == "Bar"
        }
      )
      #expect(
      ...
      #expect(
        plannedJobsA.contains { job in
          job.commandLine.contains(.flag("-module-alias")) && job.commandLine.contains(.flag("Car=Bar"))
        }
      )

      // Module alias with the default scanner (driver scanner)
      var driverB = try TestDriver(
        args: [
          "swiftc",
          "-explicit-module-build",
          "-working-directory",
      ...
      #expect(
        dependencyGraphB.modules.contains { (key: ModuleDependencyId, value: ModuleInfo) in
          key.moduleName == "Bar"
        }
      )
      #expect(
  ...
  @Test(.requireFrontendArgSupport(.moduleAlias)) func moduleAliasingInterfaceWithScanDeps() async throws {
    try await withTemporaryDirectory { path in
      ...
      #expect(
        plannedJobsA.contains { job in
          job.commandLine.contains(.flag("-module-alias")) && job.commandLine.contains(.flag("Car=E"))
        }
      )
  ...
  @Test(.requireFrontendArgSupport(.moduleAlias)) func moduleAliasingWithImportPrescan() async throws {
    ...
    try withTemporaryDirectory { path in
      ...
      var scanDiagnostics: [ScannerDiagnosticPayload] = []
      let deps =
        try dependencyOracle.getImports(
          workingDirectory: path,
    ...
    try withTemporaryDirectory { path in
      ...
      var driver = try TestDriver(
        args: [
          ...
          mainSourcePath.nativePathString(escaped: false),
        ] + sdkArgumentsForTesting
      )
      ...
      #expect(
        scannerCommand.contains(subsequence: [
          "-serialize-diagnostics-path", serializedDiagnosticsOutputPath.pathString,
        ])
      )
    ...
    try await withTemporaryDirectory { path in
      ...
      var driverFoo = try TestDriver(
        args: [
          ...
          "-working-directory",
          path.nativePathString(escaped: false),
          fooSourcePath.nativePathString(escaped: false),
        ] + sdkArgumentsForTesting
      )
  ...
  @Test func dependencyImportPrescan() async throws {
    ...
    try withTemporaryDirectory { path in
      ...
      let expectedImports4 = [
        "C", "E", "G", "Swift", "SwiftOnoneSupport", "_Concurrency", "_StringProcessing", "_SwiftConcurrencyShims",
      ]
      let expectedImports5 = ["C", "E", "G", "Swift", "SwiftOnoneSupport", "_Concurrency", "_SwiftConcurrencyShims"]
      #expect(
        Set(imports.imports) == Set(expectedImports) || Set(imports.imports) == Set(expectedImports2)
          || Set(imports.imports) == Set(expectedImports3) || Set(imports.imports) == Set(expectedImports4)
    ...
    try withTemporaryDirectory { path in
      ...
      var driver = try TestDriver(
        args: [
          ...
          "-working-directory", path.nativePathString(escaped: false),
          main.nativePathString(escaped: false),
        ] + lotsOfInputs + sdkArgumentsForTesting
      )
      let scannerJob = try driver.dependencyScanningJob()

      let resolver = try ArgsResolver(fileSystem: localFileSystem)
      let (args, _) = try resolver.resolveArgumentList(
        for: scannerJob,
  ...
  @Test(.requireFrontendArgSupport(.clangScannerModuleCachePath)) func dependencyScanningSeparateClangScanCache()
    async throws
  {
    try withTemporaryDirectory { path in
      let scannerCachePath: AbsolutePath = path.appending(component: "ClangScannerCache")
      ...
      let scannerJob = try driver.dependencyScanningJob()
      expectCommandLineContains(
        scannerJob.commandLine,
        .flag("-clang-scanner-module-cache-path"),
      ...
      do {
        ...
        #expect(
          driver.diagnosticEngine.diagnostics.contains {
            $0.behavior == .warning
              && $0.message.text
                == "In-process dependency scan query failed due to incompatible libSwiftScan (\(dummyBrokenDylib.nativePathString(escaped: false))). Fallback to `swift-frontend` dependency scanner invocation. Specify '-nonlib-dependency-scanner' to silence this warning."
          }
        )
        #expect(
          interModuleDependencyGraph.mainModule.directDependencies?.contains(where: { $0.moduleName == "C" }) == true
        )
      }
      ...
      do {
        ...
        #expect(
          !driver.diagnosticEngine.diagnostics.contains {
            $0.behavior == .error
              && $0.message.text
                == "Swift Caching enabled - libSwiftScan load failed (\(dummyBrokenDylib.nativePathString(escaped: false))."
          }
        )
      }
    }
  ...
  @Test(.requireScannerSupportsPerScanDiagnostics()) func parallelDependencyScanningDiagnostics() async throws {
    ...
    try withTemporaryDirectory { path in
      ...
      for fileIndex in 0..<numFiles {
        ...
        try localFileSystem.writeFileContents(
          files.last!,
          bytes: ByteString(encodingAsUTF8: "import UnknownModule\(fileIndex);")
        )
        var driver = try TestDriver(
          args: baseDriverArgs + [files.last!.nativePathString(escaped: false)] + [
  ...
  @Test func printingExplicitDependencyGraph() async throws {
    try await withTemporaryDirectory { path in
      ...
      let cHeadersPath: AbsolutePath = try testInputsPath.appending(component: "ExplicitModuleBuilds").appending(
        component: "CHeaders"
      )
  ...
  @Test func dependencyGraphDotSerialization() async throws {
    ...
    try withTemporaryDirectory { path in
      ...
      #expect(
        contents.contains("\"Swift\" [style=bold, color=orange, style=filled")
      ...
      #expect(
        contents.contains("\"Swift\" -> \"SwiftShims (C)\" [color=black];")
  ...
  @Test func dependencyScanCommandLineEscape() throws {
    ...
    let scanJobCommand = try Driver.itemizedJobCommand(
      of: scanJob,
      useResponseFiles: .disabled,
  ...
  @Test func explicitSwiftModuleMap() throws {
    let jsonExample: String = """
      ...
      """
    let moduleMap = try JSONDecoder().decode(
      Array<SwiftModuleArtifactInfo>.self,
    ...
    try await withTemporaryDirectory { path in
      ...
      var driver = try TestDriver(
        args: [
          ...
          fileA.nativePathString(escaped: false), fileB.nativePathString(escaped: false),
        ] + sdkArgumentsForTesting
      )
      let jobs = try await driver.planBuild()
      let compileJobs = jobs.filter({ $0.kind == .compile })
  ...
  @Test func clangTargetOptionsExplicit() async throws {
    ...
    do {
      try await withTemporaryDirectory { path in
        ...
        #expect(
          emitModuleJob.commandLine.contains(subsequence: [.flag("-clang-target"), .flag("x86_64-apple-macosx10.15")])
        )
      }
    }
    ...
    do {
      try await withTemporaryDirectory { path in
        ...
        expectCommandLineContains(emitModuleJob.commandLine, .flag("-sdk"))
        #expect(
          emitModuleJob.commandLine.contains(subsequence: [.flag("-clang-target"), .flag("x86_64-apple-macosx10.12")])
        )
      }
    }
    ...
    do {
      try await withTemporaryDirectory { path in
        ...
        #expect(
          emitModuleJob.commandLine.contains(subsequence: [
            .flag("-clang-target-variant"), .flag("x86_64-apple-ios13.1-macabi"),
          ])
        )
      }
    }
    ...
    do {
      try await withTemporaryDirectory { path in
        ...
        #expect(
          emitModuleJob.commandLine.contains(subsequence: [
            .flag("-clang-target-variant"), .flag("x86_64-apple-ios14.0-macabi"),
          ])
        )
      }
    }
      ...
      try await withTemporaryDirectory { path in
        ...
        #expect(
          targetModuleJob.commandLine.contains(
            .path(.relative(try .init(validating: "foo.swiftmodule/target.swiftdoc")))
          )
        )
        #expect(
          targetModuleJob.commandLine.contains(
        ...
        #expect(
          targetModuleJob.commandLine.contains(
            .path(.relative(try .init(validating: "foo.swiftmodule/target.abi.json")))
          )
        )
        #expect(
        ...
        #expect(
          verifyTargetModuleJob.commandLine.contains(
            .path(.relative(try .init(validating: "foo.swiftmodule/target.swiftinterface")))
          )
        )
        ...
        #expect(
          verifyVariantModuleJob.commandLine.contains(
            .path(.relative(try .init(validating: "foo.swiftmodule/variant.swiftinterface")))
          )
        )

        let interfaceCompilationJobs = try plannedJobs.findJobs(.compileModuleFromInterface)
        let _ = try #require(
          interfaceCompilationJobs.first {
            $0.commandLine.contains(subsequence: [.flag("-module-name"), .flag("A")])
        ...
        let _ = try #require(
          interfaceCompilationJobs.first {
            $0.commandLine.contains(subsequence: [.flag("-module-name"), .flag("G")])
              && $0.commandLine.contains(subsequence: [.flag("-target"), .flag("x86_64-apple-macosx10.14")])
          }
        )
        let _ = try #require(
        ...
        let _ = try #require(
          interfaceCompilationJobs.first {
            $0.commandLine.contains(subsequence: [.flag("-module-name"), .flag("E")])
              && $0.commandLine.contains(subsequence: [.flag("-target"), .flag("x86_64-apple-macosx10.14")])
          }
        )
        let _ = try #require(
        ...
        let _ = try #require(
          pcmCompilationJobs.first {
            $0.commandLine.contains(subsequence: [.flag("-module-name"), .flag("A")])
              && $0.commandLine.contains(subsequence: [
                .flag("-Xcc"), .flag("-triple"), .flag("-Xcc"), .flag("x86_64-apple-macosx12.14.0"),
        ...
        let _ = try #require(
          pcmCompilationJobs.first {
            $0.commandLine.contains(subsequence: [.flag("-module-name"), .flag("A")])
              && $0.commandLine.contains(subsequence: [
                .flag("-Xcc"), .flag("-darwin-target-variant-triple"), .flag("-Xcc"),
                .flag("x86_64-apple-ios15.1-macabi"),
              ])
          }
        )
        ...
        let _ = try #require(
          pcmCompilationJobs.first {
            $0.commandLine.contains(subsequence: [.flag("-module-name"), .flag("C")])
              && $0.commandLine.contains(subsequence: [
                .flag("-Xcc"), .flag("-triple"), .flag("-Xcc"), .flag("x86_64-apple-macosx12.14.0"),
              ]) && !$0.commandLine.contains(.flag("-darwin-target-variant-triple"))
          }
  ...
  @Test func prebuiltModuleGenerationJobs() throws {
    ...
    func checkInputOutputIntegrity(_ job: Job) {
      ...
      job.inputs.forEach { input in
        ...
        #expect(input.file.extension == "swiftmodule" || input.file.extension == "swiftinterface")
    ...
    func findJob(_ jobs: [Job], _ module: String, _ basenameWithoutExt: String) -> Job? {
      return jobs.first { job in
        return job.moduleName == module && job.outputs[0].file.basenameWithoutExt == basenameWithoutExt
      }
    }
    ...
    try withTemporaryDirectory { path in
      ...
      let (jobs, danglingJobs) = try driver.generatePrebuiltModuleGenerationJobs(
        with: interfaceMap,
        into: path,
        exhaustive: true
      )
      ...
      #expect(
        danglingJobs.allSatisfy { job in
    ...
    try withTemporaryDirectory { path in
      ...
      let (jobs, _) = try driver.generatePrebuiltModuleGenerationJobs(
        with: interfaceMap,
        into: path,
        exhaustive: false
      )
  ...
  @Test func ignoreScannerPrefixMapping() async throws {
    try await withTemporaryDirectory { path in
      ...
      #expect(
        driver.diagnosticEngine.diagnostics.contains {
          $0.behavior == .warning
            && $0.message.text == "ignore '-scanner-prefix-*' options that cannot be used without compilation caching"
        }
      )
    }
  }
  ...
  @Test func aBICheckWhileBuildingPrebuiltModule() throws {
    ...
    let collector = try SDKPrebuiltModuleInputsCollector(
      VirtualPath(path: mockSDKPath).absolutePath!,
      DiagnosticsEngine()
    )
    let interfaceMap = try collector.collectSwiftInterfaceMap().inputMap
    try withTemporaryDirectory { path in
  ...
  @Test func prebuiltModuleInternalSDK() throws {
    ...
    let collector = try SDKPrebuiltModuleInputsCollector(
      VirtualPath(path: mockSDKPathStr).absolutePath!,
      DiagnosticsEngine()
    )
    let interfaceMap = try collector.collectSwiftInterfaceMap().inputMap
    try withTemporaryDirectory { path in
