  ...
  func testImmediateMode() throws {
    do {
      ...
      if !driver.targetTriple.isWindows {
        ...
        XCTAssertFalse(job.extraEnvironment.keys.contains(ProcessEnvironmentKey(envVar)))
        #else
        XCTAssertTrue(job.extraEnvironment.keys.contains(ProcessEnvironmentKey(envVar)))
        #endif
      }
    }
