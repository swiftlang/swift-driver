...
def handle_invocation(args):
    ...
    if args.action == "build":
        if (
            ...
            "-macosx" not in args.cross_compile_hosts[0]
        ):
...
def build_swift_driver_using_cmake(
    ...
    flags = [
        "-DLLBuild_DIR=" +
        os.path.join(
            os.path.join(dependencies_dir, "llbuild"), "cmake/modules"
        ),
        "-DTSC_DIR=" +
        os.path.join(
            os.path.join(dependencies_dir, "swift-tools-support-core"),
            "cmake/modules",
        ),
        "-DArgumentParser_DIR=" +
        os.path.join(
            os.path.join(dependencies_dir, "swift-argument-parser"),
            "cmake/modules",
        ),
    ]
...
def main():
    ...
    if (
        args.build_target == "x86_64-apple-macosx" and
        "macosx-arm64" in args.cross_compile_hosts
    ):
        ...
        args.build_target == "arm64-apple-macosx" and
        "macosx-x86_64" in args.cross_compile_hosts
    ):
        args.cross_compile_hosts = [
            args.build_target + macos_deployment_target,
